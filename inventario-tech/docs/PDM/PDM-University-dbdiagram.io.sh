...
Table velaris.system_users {
  ...
  role          varchar(20)  [not null, default: 'seller', note: 'admin, warehouse_manager, seller']
...
Table velaris.purchase_orders {
  ...
  status      varchar(20)   [not null, default: 'pending', note: 'pending, approved, received, cancelled']
...
Table velaris.inventory_movements {
  ...
  movement_type varchar(20) [not null, note: 'entry, exit, adjustment, return']
  quantity      int         [not null]
...
Ref: "velaris"."categories"."created_at" < "velaris"."categories"."category_id"